...
public class AuthenticationUser implements IAuthenticationUser {
    ...
    public User registerNewUser(AuthRequest authRequest) {
        ...
        user = userRepository.save(user); // Thêm dòng này
        // ----------------------------------

        // Đoạn code gửi RabbitMQ nên để sau khi save thành công
        ...
        if (isFarmManager) {
            ...
            farmerData.put("id", user.getId()); // Lúc này mới có ID thực
            farmerData.put("username", user.getUsername());
            farmerData.put("email", user.getEmail());
            // Lưu ý: userProfile có thể chưa được link chặt chẽ nếu chưa save cascade, 
            // nhưng tạm thời cứ để code cũ của bạn
