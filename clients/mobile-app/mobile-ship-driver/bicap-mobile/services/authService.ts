...
export const authService = {
     ...
     * Returns token only if user has ROLE_DELIVERYDRIVER
     */
    login: async (email: string, password: string): Promise<AuthResponse> => {
        try {
            // Endpoint: POST /api/auth/login (Kong route tới auth-service)
            // Auth-service returns JWT token directly as string
            ...
            // Preserve custom error messages (like role check failure)
            if (error instanceof Error && !axiosError.response) {
                throw error;
            }
            const errorMessage = axiosError.response?.data?.message || 'Đăng nhập thất bại. Email hoặc mật khẩu không đúng.';
