    ...
    try {
      ...
      if (typeof response.data === 'string') {
        ...
        try {
          ...
          user = { email: payload.email, role: 'RETAILER', username: payload.sub };
        } catch (e) {
          user = { email, role: 'RETAILER' };
        }
      } else if (response.data?.data) {
        token = response.data.data.token || response.data.data;
        user = response.data.data.user || { email, role: 'RETAILER' };
      } else if (response.data?.token) {
        token = response.data.token;
        user = response.data.user || { email, role: 'RETAILER' };
      }
